...
window.testResolveRfaMatched = async function () {
  const claimId = "31c0ed61-e00c-42b0-9297-ed53a3b9c2a3";

  const { data: sessionData, error: sessionError } = await supabase.auth.getSession();

  if (sessionError || !sessionData?.session?.access_token) {
    console.error("Sessione non valida:", sessionError);
    alert("Sessione non valida. Fai login.");
    return;
  }
  ...
  console.log("RISPOSTA resolve-rfa-claim:", response.status, result);
  alert(`${response.status} - ${result.message || result.error || "Risposta ricevuta"}`);
